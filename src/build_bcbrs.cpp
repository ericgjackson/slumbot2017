...
int main(int argc, char *argv[]) {
  ...
  bool cfrs;
  string v_arg = argv[6];
  if (v_arg == "cbrs")      cfrs = false;
  else if (v_arg == "cfrs") cfrs = true;
  else                      Usage(argv[0]);
  unsigned int it;
  ...
  if (both || p0) {
    fprintf(stderr, "P0\n");
    BCBRBuilder builder(*card_abstraction, *betting_abstraction,
			*cfr_config, buckets, cfrs, 0, it, num_threads);
    builder.Go();
  }
  if (both || p1) {
    fprintf(stderr, "P1\n");
    BCBRBuilder builder(*card_abstraction, *betting_abstraction,
			*cfr_config, buckets, cfrs, 1, it, num_threads);
    builder.Go();
  }
}
