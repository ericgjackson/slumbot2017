...
TCFRThread::TCFRThread(const BettingAbstraction &ba, const CFRConfig &cc,
		       ...
		       unsigned int *short_uncompress,
		       unsigned int *pruning_thresholds,
  ...
  if (cv.size() == 0) {
    for (unsigned int s = 0; s <= max_street_; ++s) {
      ...
      sumprob_ceilings_[s] = 2000000000U;
    }
  } else {
...
void TCFR::ReadSumprobs(unsigned char *ptr, Node *node, Reader ***readers,
  ...
  if (num_succs > 1) {
	...
	if (boost_) {
	  ...
	  unsigned int *action_sumprobs = (unsigned int *)
	    (SUCCPTR(ptr) + num_succs * 8 + num_buckets * num_succs *
	     (sizeof(T_REGRET) + sizeof(T_SUM_PROB)));
	  for (unsigned int s = 0; s < num_succs; ++s) {
	    action_sumprobs[s] = succ_total_sumprobs[s];
#if 0
	    if (st == 0 && pa == 1 && node->NonterminalID() == 0) {
	      fprintf(stderr, "asp[%u] %llu\n", s,
		      succ_total_sumprobs[s]);
	    }
#endif
	  }
	}
...
void TCFR::Read(unsigned int batch_base) {
  ...
  for (unsigned int p = 0; p < num_players; ++p) {
	...
	suffix = 'i';
      ...
      sprintf(buf, "%s/regrets.x.0.0.%u.%u.p%u.%c", dir,
	      st, batch_base, p, suffix);
