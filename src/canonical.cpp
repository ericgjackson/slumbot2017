...
static unsigned int EncodeRanksOfRawSuitForStreet(unsigned int raw_suit,
						  unsigned int street,
						  const Card *raw_cards) {
  unsigned int street_code = 0;
  unsigned int num_cards_for_street = Game::NumCardsForStreet(street);
...
void CanonicalizeCards(const Card *raw_board, const Card *raw_hole_cards,
  ...
  for (unsigned int s = 0; s < 4; ++s) {
    suit_codes[s] = EncodeRanksOfRawSuit(s, raw_board, raw_hole_cards,
					 max_street);
  }
  ...
  if (raw_hole_cards) {
    ...
    for (unsigned int i = 0; i < num_hole_cards; ++i) {
      ...
      canon_hole_cards[i] = MakeCard(Rank(raw_card),
				     suit_mapping[Suit(raw_card)]);
    }
