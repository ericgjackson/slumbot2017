...
unsigned int Params::GetParamIndex(const char *name) const {
  unsigned int i;
  unsigned int num_params = param_names_.size();
...
void Params::ReadFromFile(const char *filename) {
  ...
  while (reader.GetLine(&line)) {
    ...
    unsigned int j = GetParamIndex(param_name.c_str());
...
string Params::GetStringValue(const char *name) const {
  unsigned int i = GetParamIndex(name);
