...
CanonicalCards::CanonicalCards(unsigned int n, const Card *previous,
  ...
  unsigned int num_remaining = Game::NumCardsInDeck() - num_previous;
  num_canon_ = 0;
  if (n == 1) {
    cards_.reset(new Card[num_remaining]);
    num_variants_.reset(new unsigned char[num_remaining]);
    canon_.reset(new unsigned int[num_remaining]);
    if (maintain_suit_groups) {
      suit_groups_.reset(new unsigned int[num_remaining]);
    }
    Card canon_cards[1];
    for (unsigned int c = 0; c <= max_card; ++c) {
      if (InCards(c, previous, num_previous)) continue;
      cards_[index] = c;
      unsigned int num_mappings = NumMappings(&cards_[index], n,
    ...
    unsigned int num = num_remaining * (num_remaining - 1) / 2;
    cards_.reset(new Card[2 * num]);
    num_variants_.reset(new unsigned char[num]);
    canon_.reset(new unsigned int[num]);
    if (maintain_suit_groups) {
      suit_groups_.reset(new unsigned int[num]);
    }
	...
	cards_[index * 2] = hi;
	cards_[index * 2 + 1] = lo;
	unsigned int num_mappings = NumMappings(&cards_[index * 2], n,
						previous_suit_groups);
	num_variants_[index] = num_mappings;
	unsigned short encoding;
	if (num_mappings == 0) {
    ...
    Card canon_cards[3];
    for (unsigned int hi = 2; hi <= max_card; ++hi) {
      if (InCards(hi, previous, num_previous)) continue;
      for (unsigned int mid = 1; mid < hi; ++mid) {
	if (InCards(mid, previous, num_previous)) continue;
	for (unsigned int lo = 0; lo < mid; ++lo) {
	  if (InCards(lo, previous, num_previous)) continue;
	  cards_[index * 3] = hi;
	  cards_[index * 3 + 1] = mid;
	  cards_[index * 3 + 2] = lo;
	  unsigned int num_mappings = NumMappings(&cards_[index * 3], n,
						  previous_suit_groups);
	  num_variants_[index] = num_mappings;
...
bool CanonicalCards::ToCanon2(const Card *cards, unsigned int num_cards,
  ...
  for (unsigned int i = 0; i < num_cards; ++i) {
    ...
    if (new_suit != old_suit) {
      ...
      for (unsigned int j = i + 1; j < num_cards; ++j ) {
    ...
    /* Update suit_groups */
    for (unsigned int m = 0; m < num_suits; ++m) {
      unsigned int n;
      for (n = 0; n < m; ++n) {
...
void CanonicalCards::SortByHandStrength(const Card *board) {
  unsigned int num_board_cards = Game::NumBoardCards(Game::MaxStreet());
  unique_ptr<Card []> sorted_board(new Card[num_board_cards]);
  for (unsigned int i = 0; i < num_board_cards; ++i) sorted_board[i] = board[i];
  qsort((void *)sorted_board.get(), (size_t)num_board_cards, sizeof(Card),
	g_ui_larger_compare);
#if 0
  for (unsigned int j = 0; j < num_board_cards; ++j) {
    for (unsigned int k = j + 1; k < num_board_cards; ++k) {
      if (sorted_board.get()[j] == sorted_board.get()[k]) {
	fprintf(stderr, "Illegal board\n");
	for (unsigned int l = 0; l < num_board_cards; ++l) {
  ...
  for (unsigned int i = 0; i < num_raw_; ++i) {
  ...
  sort(hands.begin(), hands.end(), g_hand_lower_compare);

  hand_values_.reset(new unsigned int[num_raw_]);
  Card *new_cards = new Card[num_raw_ * n_];
  unsigned char *new_num_variants = new unsigned char[num_raw_];
  unsigned int *new_canon = new unsigned int[num_raw_];

  for (unsigned int i = 0; i < num_raw_; ++i) {
    unsigned int hv = hands[i].hv;
    unsigned int index = hands[i].index;
...
unsigned int CanonicalCards::NumMappings(const Card *cards,
					 unsigned int num_cards,
					 unsigned int old_suit_groups) {
  unsigned int group_used[4], suit_used[4], group_size[4];
  unsigned int num_suits = Game::NumSuits();
  // Start with all suits in group 0
  unsigned int new_suit_groups = old_suit_groups;
  for (unsigned int s = 0; s < num_suits; ++s) group_size[s] = 0;
  for (unsigned int s = 0; s < num_suits; ++s) {
    ++group_size[((unsigned char *)&new_suit_groups)[s]];
  }
  unsigned int start = 0;
  unsigned int num_mappings = 1;
...
void UpdateSuitGroups(const Card *cards,
  ...
  unsigned int seen_ranks[4];
  unsigned int num_suits = Game::NumSuits();
  // Default of 0
  *new_suit_groups = 0;
  for (unsigned int s = 0; s < num_suits; ++s) seen_ranks[s] = 0;
  for (unsigned int i = 0; i < num_cards; ++i) {
    Card c = cards[i];
    char rank = Rank(c);
    int suit = Suit(c);
  ...
  for (unsigned int s2 = 1; s2 < num_suits; ++s2) {
    unsigned int sr2 = seen_ranks[s2];
    unsigned int s1;
    for (s1 = 0; s1 < s2; ++s1) {
